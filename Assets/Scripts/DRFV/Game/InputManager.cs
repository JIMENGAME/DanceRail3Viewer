...
namespace DRFV.Game
{
    ...
    {
        public static int TOUCH_MAX = 20;
        ...
        [SerializeField] SpriteRenderer[] Beams;
        ...
        void Update()
        {
            ...
            if (Input.touchCount > 0)
            {
                ...
                {
                    Vector2 pos = new Vector2(curve.Evaluate(Input.GetTouch(cnt).position.x),
                        Input.GetTouch(cnt).position.y);
            ...
            for (int i = 0; i < TOUCH_MAX; i++)
            {
                if (touchPoint[i].isTouch && oldPoint[i].isTouch)
                {
                    if (touchPoint[i].posx > left - 1 && touchPoint[i].posx < right + 1)
                    {
                        ...
                        if (Mathf.Abs(oldPoint[i].posy - touchPoint[i].posy) > 0.1f)
                        {
                            return true;
                        }
                    }
