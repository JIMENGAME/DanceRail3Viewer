...
namespace DRFV.Game.SceneControl
{
    public class Aegleseeker : global::DRFV.Game.SceneControl.SceneControl
    {
        ...
        public void Init(TheGameManager theGameManager, float time, Transform aegleseekerObject)
        {
            ...
            AnimationCurve BPMCurve = theGameManager.BPMCurve;
            FadeStart = BPMCurve.Evaluate(time);
            FadeEnd = BPMCurve.Evaluate(time + 8);
            DarkStart = BPMCurve.Evaluate(time + 13.25f);
            DarkEnd = BPMCurve.Evaluate(time + 13.5f);
            DFStartAnd = BPMCurve.Evaluate(time + 13.875f);
            AndIn = BPMCurve.Evaluate(time + 14f);
            AndInThat = BPMCurve.Evaluate(time + 14.25f);
            AndInThatLight = BPMCurve.Evaluate(time + 14.5f);
            AndInThatLightI = BPMCurve.Evaluate(time + 14.625f);
            EndAndInThatLighI = BPMCurve.Evaluate(time + 14.75f);
            DFP1AndInThatLightIFind = BPMCurve.Evaluate(time + 14.875f);
            EndAndInThatLightIFind = BPMCurve.Evaluate(time + 15f);
            DFP2AndInThatLightIFindDe = BPMCurve.Evaluate(time + 15.25f);
            AndInThatLightIFindDeliver = BPMCurve.Evaluate(time + 15.5f);
            DarkFadeEnd = BPMCurve.Evaluate(time + 15.75f);
            TextEnd  = BPMCurve.Evaluate(time + 15.875f);
